...
export default function AIWebScraper() {
    ...
    const handleScrape = async () => {
        ...
        try {
            ...
            switch (mode) {
                case 'smart':
                    endpoint = 'http://http://13.218.77.175/:3000/api/scrape/smart';
                    ...
                    break;
                case 'custom':
                    endpoint = 'http://http://13.218.77.175/:3000/api/scrape';
                    body.prompt = prompt;
                    break;
                case 'product':
                    endpoint = 'http://http://13.218.77.175/:3000/api/scrape/products';
                    break;
                case 'article':
                    endpoint = 'http://http://13.218.77.175/:3000/api/scrape/article';
                    break;
                default:
            ...
            setError('Failed to connect to scraper API. Make sure the server is running on http://http://13.218.77.175/:3000');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDeploy = async () => {
        ...
        try {
            const response = await fetch('http://http://13.218.77.175/:3000/api/deploy', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
